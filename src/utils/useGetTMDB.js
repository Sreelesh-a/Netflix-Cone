...
//   const aiMovieSuggestionCheck =useSelector(store=>store.user.AiMovieSuggestions)
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://api.themoviedb.org/3/search/movie?query=${props}&include_adult=false&language=en-US&page=1`,
          TMDB_API_OPTIONS
        ).then((res) => res.json());
